...
from celery_worker.tasks import test
...
def sort_playlist(self):
    ...
    try:
        ...
        try:
            ...
            self.update_state(state='PROGRESS', meta={'current': 2})
            try: 
                #yield f'data: Sorting Playlist... \n\n'
                ...
                self.update_state(state='PROGRESS', meta={'current': 3})
                try:
                    #yield f'data: Updated Playlist on Youtube... \n\n'
                    ...
                    self.update_state(state='PROGRESS', meta={'current': 4})
                    try:
                        #yield f'data: Storing Data in Database... \n\n'
                        ...
                        self.update_state(state='PROGRESS', meta={'current': 5})
                    except Exception as e:
                        #yield f'data: Error Storing Data! \n\n'
                        sortLog.error(f"Error: {e}")
                except Exception as e:
                    #yield f'data: Error Updating Playlist on Youtube! \n\n'
                    sortLog.error(f"Error: {e}")
            except Exception as e:
                #yield f'data: Error Sorting Playlist! \n\n'
                sortLog.error(f"Error: {e}")
        except Exception as e:
            #yield f'data: Error Getting Playlist From Youtube! \n\n'
            sortLog.error(f"Error: {e}")
    except Exception as e:
        #yield f'data: Error Getting Data From Database! \n\n'
        sortLog.error(f"Error: {e}")
    try:
        pt.setQuotaUsed(inDB, quota, 1)
        sortLog.info(f"Used Quota set! Total accrude: {quota}")
        sortLog.info("Watch later stats stored")
        #yield f'data: Playlist Updated! Quota Accruded: {quota} \n\n'
        pt.CloseDBconnnection()
        sortLog.info(f"Database connection closed!")
        self.update_state(state='PROGRESS', meta={'current': 6})
    except Exception as e:
        sortLog.error(f"Error: {e}")    
    return {'status': 'done'}
...
def task_status(task_id):
    ...
    return jsonify({
        "task_id": task_id,
        "state": task.state,
        "info": str(task.info)  # contains any return value or exception info
    })
